...
function Track({ brands, reverse = false }: { brands: Brand[]; reverse?: boolean }) {
  const style = reverse ? { animationDirection: 'reverse', animationDuration: '31s' } : undefined;
  return (
    <div className="flex w-max shrink-0 animate-marquee hover:[animation-play-state:paused]" style={style}>
      {[0, 1].map((dup) => (
        <ul key={dup} className="flex shrink-0 items-center gap-16 pr-16 md:gap-[76px] md:pr-[76px]" aria-hidden={dup === 1}>
          {brands.map((brand, i) => (
            <li key={i}>
              <img
                src={`/brands/${brand.file}`}
                alt={brand.name}
                className="h-10 w-auto opacity-65 brightness-0 invert transition-opacity duration-300 hover:opacity-90 md:h-[46px]"
              />
            </li>
          ))}
        </ul>
...
export default function LogoMarquee() {
  return (
    <section id="providers" className="relative overflow-hidden px-0 pb-14 pt-12">
      <p className="mb-11 text-center text-[10px] font-semibold uppercase tracking-[0.18em] text-primary/60">Compatible with leading AI providers and tools</p>
